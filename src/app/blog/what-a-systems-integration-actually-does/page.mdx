import imageTravisHayes from '@/images/travis-hayes.jpg'

export const article = {
  date: '2026-06-24',
  title: 'What a systems integration actually does (in plain English)',
  description:
    'The most valuable software project most small businesses can buy is one nobody will ever see. Here is what an integration is, when you need one, and what to watch out for.',
  author: {
    name: 'Travis Hayes',
    role: 'Founder',
    image: { src: imageTravisHayes },
  },
}

export const metadata = {
  title: article.title,
  description: article.description,
}

Ask most business owners what software they'd love to have, and they describe something visible: a better website, a slick app, a dashboard. Almost nobody says "I'd like my systems to talk to each other." Yet in over a decade of building business software, the projects with the best payoff, dollar for dollar, have almost all been integrations.

## The problem, in one sentence

Your business runs on several systems, and the thing connecting them is a person with a keyboard.

The website takes orders, the accounting software makes invoices, the inventory lives in the point-of-sale, and the customer list lives in a CRM. None of them were designed to cooperate. So your team does what teams do: they export, they copy, they re-type. Every day.

That human bridge has three costs. It's slow: hours per week of pure typing. It's error-prone: every re-keyed order is a chance to ship the wrong thing. And it doesn't scale: double your orders and you double the typing, which means hiring someone to do work a computer should do.

## What an integration is

An integration is a small piece of software whose only job is to move information between your systems automatically and correctly.

Order comes in on the website; it appears in your fulfillment system. Invoice gets paid; your accounting software records it. Stock runs low; the count updates everywhere at once. Nobody touched a keyboard.

That's the whole idea. It isn't glamorous, and that's the point: a good integration is invisible. Things that used to be "someone's job" simply happen.

## When it's worth the money

The math is usually simple. Add up the hours per week your team spends moving data between systems, add something for the errors (a mis-shipped order, a missed invoice, an inventory write-off), and compare it to the one-time cost of connecting the systems.

For many small businesses, that's a project that pays for itself inside a year and keeps paying forever. Very few visible software projects can say that.

Signs you're ready:

- Someone's job description includes copying data from one screen to another
- Your systems disagree about inventory, revenue, or customers
- Growth plans stall because "the back office can't handle more volume"
- Month-end reporting involves manually combining exports

## What to watch out for

**Beware the rip-and-replace pitch.** Some vendors answer "my systems don't talk" with "replace all your systems with ours." Occasionally that's right. Usually it's a much bigger, riskier project than connecting what you already own and what your team already knows how to use.

**Ask about failure, not just success.** The difference between a good integration and a time bomb is what happens when something goes wrong. Good ones log what they do, retry sensibly, and tell a human when they're stuck. If a developer can't explain their answer to "how will we know if it breaks?", keep looking.

**Insist on documentation.** An undocumented integration is a future hostage situation. Whoever builds yours should leave behind a plain-English description of what connects to what, and any developer should be able to pick it up later.

## The short version

If your team is the glue between your systems, you're paying for an integration already. You're just paying for it in salary, errors, and ceiling on growth, instead of paying once to build it.

If you want help figuring out whether the math works for your particular mess of systems, [I'll give you an honest read for free](/contact).
